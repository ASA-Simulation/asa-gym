...
SCREEN_WIDTH = 800
SCREEN_HEIGHT = 800
DEG2M = 111120  # factor to convert from degrees to meters
R = 122095.85426355
PIX2M = 3.0 * R / SCREEN_WIDTH  # factor to convert from pixels to meters
M2PIX = 1.0 / PIX2M  # factor to convert from meters to pixels
DEG2PIX = DEG2M * M2PIX  # factor to convert from degrees to pixels
...
def deg2pix(value):
    """
    Converts from degrees to pixels.
...
class DrawingUtils:
    ...
    def draw_rectangle_on_screen(window, origin, dimensions, color, width):
        ...
        dimensions_on_screen = (
            round(M2PIX * dimensions[0]),
            round(M2PIX * dimensions[1]),
        )
        pygame.draw.rect(
            window, color, pygame.Rect(origin_on_screen, dimensions_on_screen), width
        )
    ...
    def draw_circle_on_screen(window, center, radius, color, width):
        ...
        """
        center_on_screen = (
            int(round(M2PIX * center[0])),
            int(round(M2PIX * center[1])),
        )
    ...
    def draw_line_on_screen(window, start, end, color, width):
        ...
        start = Vector2(start[0], start[1]) + Vector2(CENTER[0], CENTER[1])
        end = Vector2(end[0], end[1]) + Vector2(CENTER[0], CENTER[1])

        pygame.draw.line(window, color, start.to_tuple(), end.to_tuple(), width)

    @staticmethod
    def draw_arc_on_screen(
        window, center, radius, start_angle, stop_angle, color, width
    ):
        """
        ...
        rectangle_on_screen = (
            top_left_on_screen[0],
            top_left_on_screen[1],
            diameter_on_screen,
            diameter_on_screen,
        )
        pygame.draw.arc(
            window, color, rectangle_on_screen, start_angle, stop_angle + 0.01, width
        )

    @staticmethod
    def draw_img_on_screen(window, img, center):
        center = Vector2(center[0], center[1]) + Vector2(CENTER[0], CENTER[1])
        window.blit(img, img.get_rect(center=center.to_tuple()))
...
class Arc:
    ...
    def interpolate(self, t):
        ...
        angle = normalize_angle(
            self.start_angle + t * normalize_angle(self.stop_angle - self.start_angle)
        )
